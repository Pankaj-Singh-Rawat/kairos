"""
This script will load the PaySim CSV, sort it by the step column (which represents hours), and yield batches.
At step == 150, it will inject covariate shift by multiplying the transaction amount by 3 for legitimate transfers.
"""
...
class DataStreamer:
    def __init__(self, file_path: str, drift_start_step: int = 150):
        ...
        self.unique_steps = self.df['step'].unique()
        self.current_index = 0

    def get_next_batch(self):
        if self.current_index >= len(self.unique_steps):
            return None  # Stream is finished in this scenario

        step = self.unique_steps[self.current_index]
    ...
    def _inject_drift(self, batch: pd.DataFrame) -> pd.DataFrame:
        ...
        batch.loc[mask, 'amount'] = batch.loc[mask, 'amount'] * 3.0
        return batch